...
#   source('tools/generate_etrs_config.R')
...
generate_etrs_config <- function(ts, date, output_dir,
  ...
  if (!has_valid_dates) {
    stop('Timeseries is missing valid_from/valid_until columns (or all NA). ',
         'Rebuild with: Rscript src/pipeline/00_build_timeseries.R --full\n',
...
generate_s232_yaml <- function(snapshot, date, policy_params, output_dir) {
  ...
  deriv_file <- here(policy_params$section_232_derivatives$resource_file)
...
generate_ieepa_reciprocal_yaml <- function(snapshot, date, policy_params, output_dir) {
  ...
  usmca_file <- here(sprintf('resources/usmca_product_shares_%s.csv', usmca_year))
...
generate_s122_yaml <- function(date, policy_params, output_dir) {
  ...
  exempt_file <- here(s122$exempt_products %||% 'resources/s122_exempt_products.csv')
  if (file.exists(exempt_file)) {
    exempt <- read_csv(exempt_file, show_col_types = FALSE,
                        col_types = cols(.default = col_character()))
...
generate_other_params_yaml <- function(date, policy_params, output_dir,
  ...
  usmca_year <- policy_params$USMCA_SHARES$year %||% '2025'
  usmca_shares_file <- here(sprintf('resources/usmca_product_shares_%s.csv', usmca_year))
...
if (sys.nframe() == 0) {
  ...
  if (length(args) < 2) {
    message('Usage: Rscript tools/generate_etrs_config.R <date> <output_dir>')
    message('  date:       Policy date (YYYY-MM-DD)')
    message('  output_dir: Directory to write ETRs config files')
    message('')
    message('Example:')
    message('  Rscript tools/generate_etrs_config.R 2026-04-01 ../Tariff-ETRs/config/baseline/2026-04-01')
    quit(status = 1)
  }
