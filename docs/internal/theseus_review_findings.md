...
- **Files:** `src/publish_internal.R:142, 282-321` · `src/model/rate_schema.R:92-93` (`build_rev_intervals`) · `src/pipeline/00_build_timeseries.R:365-367` (in-memory-only mint append)
...
- **Files:** `config/policy_params.yaml:271-274` · `src/pipeline/06_calculate_rates.R:2171-2177` (heading guard), `:1642-1644` (blanket-on-auto-only filter) · `src/model/authority_adapter.R:122-127`
...
- **Files:** `src/scenario_ops.R:343-375` (op_set_stacking) · `src/pipeline/09_daily_series.R:131,155,173,192,220-247,267,290` (specs-less re-stack) · `src/model/stacking.R:296,303-304,350`
...
- **Files:** `src/scenario_ops.R:329-341` (set_active) · `src/model/stacking.R:213/219` reads only authority-level `$stacking$class`
...
- **Files:** `src/model/authority_adapter.R:314-335` · consumed at `06:1043-1047,1215,1286`
...
- **File:** `src/pipeline/06_calculate_rates.R:1216-1220, 1287-1291`
...
- **Files:** `src/model/data_loaders.R:93-107` · called `authority_adapter.R:618`
...
- **File:** `src/model/authority_spec.R:264-275` — NA-base guard only catches the scalar case; a vectorized base with embedded NAs → `pmax`→NA → `out[is.na(out)]<-0`.
...
- **Severity: LOW** *(corrected down from medium)* · **Confirmed** · Introduced by refactor
- **File:** `src/model/authority_spec.R:205-233` — all 4 live callers pass `product=NULL`; live 301 tier read directly at `06:~2319`; 232 deals via `s232_deal_records`.
...
- **File:** `src/pipeline/09_daily_series.R:561-564`
...
- **Severity: LOW** · **Confirmed** · Introduced by refactor
- **File:** `src/model/stacking.R:285-294` — omits `rate_301_cs`; dormant (`section_301_content_split_codes: []` + no live `tpc_additive` caller). `09:224-225` papers over it by zeroing.
...
- **Severity: MEDIUM** · **Confirmed** · Introduced by refactor
- **Files:** `src/pipeline/09_daily_series.R:1001-1067` (streaming) · `quality_report.R:378` — equivalence rests on one-off Slurm job `13800647` + a golden re-frozen *after* the streaming work landed.
...
- **Severity: LOW** *(corrected down from medium)* · **Confirmed** · Introduced by refactor
- **File:** `src/core/parity.R:94-105, 144-146` — skips `^weighted_etr|^etr_|_imports_b$`; standard gate runs `--unweighted` vs a WEIGHTED golden.
