...
In `src/model/authority_spec.R`: the `rate` sub-schema (decision 5/6), `resolve_rate()` (the
...
- **4a — Section 232** (multi-program): structure each of the **7** programs' rate
  ...
  - **Gate-tooling fix landed (commit `134759f`, src/core/parity.R):** the `--unweighted` build drops the
...
> src/model/data_loaders.R) and writes an authority-level `section_232$annex = {tier, flat_rate, floor_rate, country_overrides}`; the calculator READS it — the inline classification, the config `case_when`
...
- **P2-1 — unified timeline splitter. CODE LANDED on `theseus` (golden re-freeze pending).**
  ...
  (`src/model/timeline.R`) finds every boundary strictly interior to a real interval that the calc
  re-resolves on an as-of recompute, and `build_boundary_mints()` (`src/pipeline/00_build_timeseries.R`)
...
- **2026-06-05 — Plank 4c S1+S2a DONE (parity GREEN 47/47; commit `143a2b1`).** §232 annex regime de-blobbed to the
  ...
  the calc. Built: **(S1)** `classify_s232_annex()` extracted to `src/model/data_loaders.R` (single source of truth; arm-order
...
- **2026-06-05 — Plank 4a S1a + S1b landed (both parity GREEN 47/47); gate-tooling bug fixed.**
  ...
  (47/47); S1b array `13809706`→compare `13810632` (47/47). **Gate fix** (`134759f`, src/core/parity.R): the
...
- **2026-06-04 — Plank 0 landed (verified).** Compositional rate schema +
  `resolve_rate`/`apply_rate_semantics`/`validate_rate` in `src/model/authority_spec.R`; tests
