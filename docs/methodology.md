...
- **Exemption annex (note 50(a)(ii)–(v) / headings 9903.05.03–.06).** 2,126 HTS-8 provisions in `resources/s301_brazil_exempt_products.csv`, extracted by `scripts/build_s301_brazil_annex.R` from the Federal Register XML: a flat subheading list (864), eleven "particular articles", a civil-aircraft-use list (546) and a pharmaceutical-use list (705). The final annex is substantially broader than the June proposal (1,698): 429 provisions added (pig iron, iron/steel scrap, organic honey, unflavored instant coffee, used clothing, aluminum hydroxide, hides/furskins, additional seafood, wood and pharmaceutical products, antiques/art), and exactly one removed (high-purity dissolving pulp, 4702.00.00). The aircraft and pharma lists are use-conditional in law; the HTS-8-grained model takes them as flat exemptions (the same granularity limit as the forced-labor Annex A).
