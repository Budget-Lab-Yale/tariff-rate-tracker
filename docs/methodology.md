...
The tracker constructs statutory U.S. tariff rates at the `HTS-10 x country` level by processing USITC HTS revisions. The current repo covers 44 revisions from January 1, 2025 through July 1, 2026, and extends the final interval through December 31, 2026 using the configured series horizon.
...
**Which schedule column the base rate comes from.** Almost every origin is priced at **column 1 general** — the normal-trade-relations (MFN) rate in the HTS JSON `general` field. Four origins are instead priced at **column 2** (the JSON `other` field), because HTSUS General Note 3(b) denies them NTR treatment: Cuba and North Korea throughout, Russia and Belarus since PL 117-110 (2022-04-09, i.e. for the whole 2025–2026 series). The panel column `base_rate_source` records which column produced each row (`col1_general` / `col2_other`), and column-2 rows are excluded from the MFN exemption-share adjustment because losing NTR treatment means losing every preference program. Column-2 rates are much more specific-duty-heavy than column 1, so under the no-AVE convention these four origins' levels are understated; see `docs/assumptions.md` §20 and `docs/statutory_deviations.md` S11.
...
- Section 301 Brazil (`rate_s301br`) is additive on the same design: U.S. note 50(a) imposes it in addition to every other chapter-99 duty, and its Section 232 interaction is the same construction-time full per-article exclusion (note 50(a)(vi); see the Section 301 Brazil section below).
...
- **Section 232 full exclusion (note 51(c) / heading 9903.03.15).** Any article *in Section 232 scope* is fully excluded — it pays zero Section 338 even on its non-metal content, and a USMCA-eligible auto part paying 0% under Section 232 is still excluded. Implemented as a per-row scope mask (`statutory_rate_232 > 0`, or an in-scope `s232_annex` tier — `annex_1a/1b/1c/3` — or a `heading_program` flag), deliberately not the content-split stacking class (which would leak the duty onto the non-metal fraction) and not `rate_232 > 0` (which would miss zero-paying USMCA parts). `annex_2` articles are *removed from* Section 232 scope by the April 2026 annex and are no longer "provided for" in the 9903.82 headings note 51(c)(1) cites, so they still pay Section 338 — this covers 54 Canada lines on the covered lists, including beer 2203.00.00 (a former aluminum-can derivative that the alcohol proclamation lists explicitly). The mask is self-updating: when the pharma Section 232 heading activates on 2026-09-29, patented-pharma rows drop out of Section 338, matching note 51(c)(8)'s 9903.04.60–66 exclusion (no covered-list code is pharma, so this is currently a no-op).
- **GN6 civil aircraft (note 51(d) / heading 9903.03.16).** The 554-code note 51(d) list overlaps the covered lists on 28 HTS-8 codes (all on the motor-vehicles list). The exemption is use-conditional (GN6-certified entries only), so overlap lines are scaled by `1 − GN6 utilization share`, with the share resolved measured HTS10 → 0 (unmeasured). Measured shares come from the same IMDB measurement as the Section 122 aircraft scaling (`resources/s122_aircraft_utilization.csv`; 24 of the 28 codes are measured, so 4 fall to the 0 fallback and pay the full duty). Unlike Section 122, there is **no HS2-mean interpolation tier** and the unmeasured fallback is 0, not full exemption: Section 122's exempt set is genuinely civil-aircraft, but Section 338's covered∩GN6 overlap is consumer goods (routers, furniture, cameras) that merely share an HTS-8 with the aircraft list, so an unmeasured overlap code is treated as fully dutiable rather than interpolated from the aircraft-heavy Section 122 chapter mean (which would understate the duty ~9pp on chs 84/85).
- **Chapter 98.** Note 51(a) carries a chapter-98 paragraph, but because Section 338 attaches only on covered HTS-8 codes (all in chapters 4–97), `rate_s338` is zero on every chapter-98 row *before* the shared ch98 secondary-classification zeroing and 9802 value-basis steps run — so those steps are no-ops for `rate_s338` (it is listed in their column sets for uniformity only). Note 51(a) does carve subchapter XXIII and subheadings 9802.00.40/.50/.60/.80 *back in* (those entries of covered Canadian goods legally pay the duty, on repair/processing/assembly value for the 9802 codes); the model does **not** charge Section 338 on those chapter-98 entries. This is a documented deviation — see `docs/statutory_deviations.md` S7. Materiality is small (real vehicles are Section 232-excluded regardless; the exposure is ch98 dairy/alcohol/mv-list entries under repair/assembly provisions).

`rate_s338` and final-action `rate_s301fl` are permanent `RATE_SCHEMA` columns:
each persists as all-zero before its effective date.

### Section 301 Brazil

The USTR Section 301 investigation of Brazil concluded in a Notice of Action (FR Doc 2026-14542, published 2026-07-20) imposing an additional 25% ad-valorem duty on all products of Brazil effective 2026-07-22, via new heading 9903.05.01 under U.S. note 50. This finalizes — with material changes — the June 4, 2026 proposed action (FR Doc 2026-11158) that the repo previously modeled as part of the `new_301` *scenario*; the final action is signed law and is therefore a **baseline** authority (`rate_s301br`), and the scenario overlay no longer carries it. Sources are archived in `data/s301_brazil/` (signed USTR PDF) and `docs/s301_brazil/` (Federal Register full-text XML, June and final).

Because no HTS archive carries the 9903.05.0x headings yet, the authority is hand-fed from `config/policy_params.yaml` (`section_301_brazil:`) plus the extracted exemption annex — the Section 338 pattern — and is date-gated in the authority adapter. The turn-on is materialized in the panel by the `bnd_2026-07-22` boundary mint (`boundary_overrides`). When a real HTS revision publishes the headings, the hand-fed treatment should be reconciled against the parse.

Mechanics:

- **Rate and scope.** +25 percentage points (`rate_s301br`) on every Brazil (census 3510) row not exempted below. Brazil is not a USMCA party (`usmca_treatment = 'none'`).
- **Exemption annex (note 50(a)(ii)–(v) / headings 9903.05.03–.06).** 2,126 HTS-8 provisions across three resource files, extracted by `scripts/build_s301_brazil_annex.R` from the Federal Register XML and kept separate because their legal force differs: the **unconditional** exclusions (`s301_brazil_exempt_products.csv`, 875 = the flat subheading list 864 + eleven "particular articles") are fully exempt; the **civil-aircraft-use** list (`s301_brazil_aircraft_products.csv`, 546) and the **pharmaceutical-use** list (`s301_brazil_pharma_products.csv`, 705) are exempt only for qualifying end use, so covered lines pay the 25% scaled by (1 − utilization share) — 90% aircraft / 50% pharma in `policy_params.yaml`, backed out of GTA's published effective rates for the final action (2.5% and 12.5%). The final annex is substantially broader than the June proposal (1,698): 429 provisions added (pig iron, iron/steel scrap, organic honey, unflavored instant coffee, used clothing, aluminum hydroxide, hides/furskins, additional seafood, wood and pharmaceutical products, antiques/art), and exactly one removed (high-purity dissolving pulp, 4702.00.00). Notably, 364 pharma lines that were unconditional in the June proposal are use-conditional in the final action — the share-scaled treatment preserves that deliberate narrowing, which a flat exemption would silently undo.
- **Section 232 full exclusion (note 50(a)(vi) / heading 9903.05.07).** The final action replaces the June proposal's blanket "articles subject to Section 232" carve-out with an enumerated full-article exclusion: articles of steel/aluminum/copper and their derivatives, passenger vehicles/light trucks and parts, medium- and heavy-duty vehicles and parts, wood products, and semiconductor articles, cited by their chapter-99 heading ranges. Implemented as the same per-row scope mask as Section 338 (`statutory_rate_232 > 0`, or an in-scope `s232_annex` tier, or `heading_program`), replacing the June coding's content-split treatment — a Section 232 article now pays zero Brazil §301 even on its non-metal content, and `annex_2` articles (removed from 232 scope) still pay. Annex I Part B adds patented pharmaceutical articles (9903.04.60–66) to the exclusion effective 2026-07-31; since that precedes the model's pharma-§232 activation (2026-09-29), the `heading_program` arm — which tags pharma rows only from that date — reproduces the legal timeline exactly.
- **Stacking.** Additive (note 50(a): products remain subject to every other subchapter-III/IV duty). In particular it stacks on top of baseline forced-labor §301 — a non-exempt Brazilian good can carry both actions.
- **Chapter 98.** Note 50(a) carries the standard chapter-98 paragraph with the 9802.00.40/.50/.60/.80 value-basis exceptions; `rate_s301br` participates in the shared ch98 zeroing and 9802 value-basis conversion.
- **Not modeled.** The one-week in-transit window (9903.05.02: loaded before 07-22 and entered before 07-29), the donations and informational-materials exemptions (9903.05.08–.09), and FTZ privileged-foreign-status timing.

`rate_s301br` is a permanent `RATE_SCHEMA` column: it persists (all-zero) in pre-2026-07-22 revisions.
