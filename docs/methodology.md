...
`rate_s338` is a permanent `RATE_SCHEMA` column: it persists (all-zero) in pre-2026-08-19 revisions, unlike the scenario-scoped `rate_s301fl` column which is dropped when inactive.
...
- **Stacking.** Additive (note 50(a): products remain subject to every other subchapter-III/IV duty). In particular it stacks on top of the *scenario* forced-labor §301 when that overlay is active — a non-exempt Brazilian good there carries 25% + 12.5%.
