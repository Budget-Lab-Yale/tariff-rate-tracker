...
| S8 | **Brazil §301 skipped mechanics** (note 50(a), FR 2026-14542): the one-week in-transit window (9903.05.02: loaded before 07-22 AND entered before 07-29), donations / informational-materials exemptions (9903.05.08–.09), personal-baggage exception, and FTZ privileged-foreign-status timing are not modeled. UNLIKE §338 (S7), the ch98 mechanics ARE live here: Brazil attaches on all non-exempt products incl. ch98 lines, so the shared ch98 secondary-classification zeroing and the 9802.00.40–.80 partial-value conversion (B2/B3, incl. B3's uncalibrated 1.0 share) genuinely bind for `rate_s301br`, matching note 50(a)'s prescribed treatment. The civil-aircraft (a)(iv) and pharma-use (a)(v) lists are USE-conditional in law but taken as FLAT hts8 exemptions (over-exempts non-qualifying entries under those lines — the FL Annex A granularity limit; unlike §338's GN6 there is no Brazil-specific utilization measurement). Five 10-digit annex provisions (incl. organic honey 0409.00.0005) are truncated to their hts8 (over-exempts sibling stat lines). Patented-pharma carve-out (Annex I Part B, eff. 07-31) rides the pharma-§232 `heading_program` arm, so it binds from the model's pharma turn-on 09-29 — no duty is legally collectible on those headings in between either way. | notice text (data/s301_brazil/) | `apply_section301_brazil()` (06); `resources/s301_brazil_exempt_products.csv` | statutory reading; flat aircraft/pharma exemption is a judgment call |
