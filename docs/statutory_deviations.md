...
| S7 | **§338 Canada skipped mechanics** (note 51(a)): the personal-baggage exception, FTZ privileged-foreign-status timing, and de minimis interactions are not modeled (negligible on the covered lists); the ch98 exception is modeled via the SHARED ch98 zeroing + 9802 value-basis conversion (B2/B3), with no §338-specific refinement. GN6 aircraft exemption is utilization-scaled with a measured → HS2-mean → **0** fallback (vs §122's → full exemption): the 28 affected mv-list codes are overwhelmingly non-aircraft entries. | proclamation text (data/s338/); IMDB GN6 measurement | `apply_section338()` (06); `resources/s338_*.csv` | statutory reading; GN6 fallback is a judgment call |
