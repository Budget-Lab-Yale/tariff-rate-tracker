...
| S10 | **Polysilicon minimum-import-price component omitted from the baseline.** The December 4 action sets $21/kg raw-polysilicon, $100/kg wafer, $0.22/watt cell, and $0.38/watt module thresholds with documentation/resale conditions. The baseline models the accompanying ad valorem headings but records the MIP contribution as zero because the value panel lacks complete kilograms/watts and importer-level compliance facts. No flat AVE is imputed. | proclamation Annex I / U.S. note 42; exact scope in `s232_polysilicon_mip_products.csv` | `section_232_headings.polysilicon.mip_model: documented_zero` | **gap**; quantity-based AVE remains a sensitivity-track task |
| S11 | **Column 2 base rates for non-NTR origins** (GN 3(b)): Cuba/North Korea always, Russia/Belarus from PL 117-110 (2022-04-09) are priced off the HTS `other` field instead of `general`. Two consequences are registered here rather than left implicit. (1) **The country side is an exogenous overlay** (`column_2_countries`), because GN 3(b) is prose and nothing in the JSON or in Census data marks NTR status — so the list can go stale silently if Congress acts and no one edits the config; it is not inferred from line data (same rule as GSP windows, decision 5 of the pre-2025 scoping doc). (2) **Column-2 levels are a LOWER BOUND under S2**: ~22% of column-2 lines are specific/compound vs ~7% of column-1 lines, and those cells are 0 with an honest `specific_or_compound` type, so these four origins' measured base rates read LOW until AVEs exist (Phase 4). Where both columns parse, mean col-1 4.4% vs mean col-2 36.5% on 2026 rev_9 and col-2 is never below col-1, so the direction is unambiguous. Also: no preference scaling on these rows (no NTR means no preference programs), which is a deliberate carve-out of U2, and flat Ch99 layers (Russia 200% aluminum, annex_1a, §122, §301) are unaffected by construction. | GN 3(b); PL 117-110; codes from `resources/census_codes.csv` | `column_2_countries:` config; `apply_column_2_rates()` + `zero_exemption_share_for_column_2()` (06 step 1c / 6c); A§20 | statutory reading live; col-2 specific-duty AVEs a **gap** (Phase 4) |
| S12 | **Quartz-surface-products §201 modelled at the in-quota rate** (U.S. note 41, Proc 11051, headings 9903.45.30/.31, effective 2026-08-15): the safeguard is a TRQ — 25% in-quota / 50% over-quota in Year 1, stepping to 19%/47% by 2029 — and the tracker carries one statutory rate per line, so `rate_section_201` takes the IN-QUOTA leg. The quota (13,006,426 m²/yr, 3,251,606 per quarter, growing annually) sits well above observed QSP import volumes, so most entries should clear in-quota; the over-quota rate is an unmodeled upper bound. Quota fill, quarterly carry-forward of unused quantity, and the m² quantity basis are not modelled — the weights pipeline carries value, not area. Note 41(c) exempts 124 economies (USMCA, FTA partners, developing countries, CBERA), which removes every QSP origin present in the repo's CA/MX trade files; the dutiable origins appear only in the full weighted build. | note 41(d)/(e) | HTS 2026 rev_16 ingest | **documented choice** |
...
| T4 | **Pharmaceutical §232 July 31–September 28 window:** the baseline assumes patented-pharmaceutical imports in this window are for Secretary-identified companies and therefore models no §232 pharmaceutical duty until September 29. The note-40(c) product universe is roughly $160B of 2024 imports, so this is explicit rather than treated as immaterial. | Company-level Secretary list/entry linkage is unavailable; imposing 100% on all trade would be the opposite unsupported extreme | `section_232_headings.pharmaceuticals.effective_date: 2026-09-29` |
...
| F9 | **§232 GN 3(b) derivative rate (heading 9903.82.12)** — from the 2026 annex regime, derivative aluminum/steel articles the product of a general-note-3(b) country carry **+25%** (note 16(c)(ix)–(x)), not the 50% tier those lines otherwise draw. Currently unmodeled, so these four origins' derivative steel/aluminum is **overstated** (Russia ch76 is unaffected in practice: the 200% surcharge of Proc. 10522 `pmax`es over any tier). Found while implementing S11 / A§20a. | gap | A§20a (2026-07-29) | needs country-aware annex tier assignment + the 9903.82.17 / 9903.85.68 exceptions; next GN 3(b) item |
