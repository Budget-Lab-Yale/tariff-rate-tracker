...
#   Rscript src/pipeline/01_scrape_revision_dates.R --dry-run  # preview without writing
#
# =============================================================================
...
update_revision_dates <- function(csv_path, api_releases, dry_run = FALSE) {
  ...
  message('  4. Run Rscript src/pipeline/02_download_hts.R to fetch the new JSON')
  message(strrep('!', 70))
...
check_chapter99_pdf_changed <- function(
  ...
  if (is.na(previous_hash)) {
    ...
    message('')
    message('  Next steps:')
    message('    1. Run: Rscript tools/scrape_us_notes.R --all')
    message('    2. Review regenerated resource files for changes')
    message('    3. Run: Rscript src/pipeline/01_scrape_revision_dates.R --accept-pdf-hash')
