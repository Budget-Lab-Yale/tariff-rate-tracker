...
source(here('src', 'policy_params.R'))  # load_policy_params -> SERIES_HORIZON_END + load_local_paths
source(here('src', 'build_panel_import_weights.R'))  # forward-mapped HS10 x country import weights
...
write_build_output <- function(shared_root = SHARED_ROOT_DEFAULT,
  ...
  # src/build_panel_import_weights.R). Keyed to the actual panel; scenarios share
