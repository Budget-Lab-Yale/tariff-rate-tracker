...
if (requireNamespace('here', quietly = TRUE)) {
  source(here::here('src', 'output_paths.R'))   # SNAPSHOT_FILE / actual_snapshots_dir
}
...
.bpiw_print_help <- function() {
  cat('Usage: Rscript src/build_panel_import_weights.R --vintage-dir <DIR> [options]\n\n')
...
if (sys.nframe() == 0) {
  ...
  if (is.null(base_path) && requireNamespace('here', quietly = TRUE)) {
    tryCatch(source(here::here('src', 'policy_params.R')),
